...
public:
    ...
    VkDebugManager(const VkInstance& instance) : instance(instance) {
...
const std::vector<const char*> VkDebugManager::validationLayers = {
...
private:
    ...
    void createInstance() {
        if (enableValidationLayers && !VkDebugManager::checkValidationLayerSupport()) {
            throw std::runtime_error("validation layers requested, but not available!");
        }

        VkApplicationInfo appInfo{};
        appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
        appInfo.pApplicationName = "Rise";
        appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
        appInfo.pEngineName = "Rise";
        ...
        if (enableValidationLayers) {
            createInfo.enabledLayerCount = static_cast<uint32_t>(VkDebugManager::validationLayers.size());
            createInfo.ppEnabledLayerNames = VkDebugManager::validationLayers.data();
    ...
    void setupDebugMessenger() {
        debugManager.reset(new VkDebugManager(instance));
    }
    ...
    void createImageViews() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            createInfo.format = swapChainImageFormat;createInfo.components.r = VK_COMPONENT_SWIZZLE_IDENTITY;
    ...
    static void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandPool commandPool, VkDevice device, VkQueue queue) {
        VkCommandBufferAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        allocInfo.commandPool = commandPool;
    ...
    VkInstance instance;
    std::unique_ptr<VkDebugManager> debugManager;
...
public:
    ...
    Window(int width, int height) : width(width), height(height) {
        ...
        _id = glfwCreateWindow(width, height, "Vulkan", nullptr, nullptr);

        if (glfwCreateWindowSurface(WindowManager::Instance().getInstance(), _id, nullptr, &surface) != VK_SUCCESS) {
            throw std::runtime_error("failed to create window surface!");
        }

        auto graphicIndices = WindowManager::Instance().findGraphicsQueueFamilies();
        auto presentIndices = WindowManager::Instance().findPresentQueueFamilies(surface);

        std::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
        std::set<uint32_t> uniqueQueueFamilies = { graphicIndices.graphicsFamily.value(), presentIndices.presentFamily.value() };
    ...
    ~Window() {
        vkDestroySurfaceKHR(WindowManager::Instance().getInstance(), surface, nullptr);
...
int main() {
    ...
    try {
        ...
        WindowManager::Instance().mainloop();
